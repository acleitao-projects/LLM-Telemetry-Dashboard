"""Nautilus agent - tiny PASSIVE telemetry helper for the host that runs
llama.cpp (expected: Linux).

Run ON Nautilus:
    python3 nautilus_agent.py            # listens on 127.0.0.1:8091
...
def system_info() -> dict:
    ...
    return {
        "hostname": platform.node() or "nautilus",
...
def main():
        ...
        def do_GET(self):
            ...
            try:
                if p == "/info":
                    ...
                    self._send({"status": "ok", "agent": "nautilus"})
                else:
    ...
    print(f"nautilus agent listening on http://{args.host}:{args.port} (passive)")
    srv.serve_forever()
